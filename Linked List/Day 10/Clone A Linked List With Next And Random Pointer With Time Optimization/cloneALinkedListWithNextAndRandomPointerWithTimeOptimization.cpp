...
node *copyList(node *head){
    ...
    //making copy of original list by using next pointer
    //and keeping random pointer NULL
    ...
    //mapping the address of original list elements and 
    //its corresponding addresses of elements of copy list
    curr=head;
    currCopy=headCopy;
    while(curr){
        address[curr]=currCopy;
        curr=curr->next;
        currCopy=currCopy->next;
    }

    //giving address in random pointer of each element in 
    // copy list by using map created above 
    curr=head;
    currCopy=headCopy;
    while(curr){
        currCopy->arb=address[curr->arb];
        currCopy=currCopy->next;
        curr=curr->next;
    }
    return headCopy;
